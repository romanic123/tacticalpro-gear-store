...
function App() {
  ...
  // Advanced: Navigation timeline arrays mapping routes
  const [pageHistory, setPageHistory] = useState(['Home']);
  const [historyIndex, setHistoryIndex] = useState(0);
  const [isInternalNavigating, setIsInternalNavigating] = useState(false);

  // Synchronize path index steps when changing views via top header links
  useEffect(() => {
  ...
  // Handle hardware popstate triggers to use our local memory array instead of leaving site
  useEffect(() => {
  ...
  return (
    <div className="bg-light min-vh-100 d-flex flex-column" style={{ paddingTop: '70px' }}>
      ...
      {/* FIXED INTERNAL HISTORY MANAGEMENT BAR */}
      ...
      {/* DYNAMIC ROW ROUTER CONTAINER VIEWS */}
